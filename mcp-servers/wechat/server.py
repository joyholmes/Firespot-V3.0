...
OPENAI_IMAGE_BASE_URL = os.environ.get("OPENAI_IMAGE_BASE_URL", "").rstrip("/")
...
DEFAULT_OUTPUT_ROOT = Path("/tmp/firespot-user-data")


def resolve_output_path(output_path: str | None) -> str:
    """将图片统一落盘到 /tmp/firespot-user-data，避免依赖 DeerFlow 物理目录映射。"""
    if not output_path:
        return str(DEFAULT_OUTPUT_ROOT / "outputs" / f"openai_image_{int(time.time())}.png")

    normalized = output_path.strip()
    if normalized.startswith("/mnt/user-data/"):
        relative_path = normalized.removeprefix("/mnt/user-data/")
        return str(DEFAULT_OUTPUT_ROOT / relative_path)

    path_obj = Path(normalized)
    if path_obj.is_absolute() and str(path_obj).startswith(str(DEFAULT_OUTPUT_ROOT)):
        return str(path_obj)

    return str(DEFAULT_OUTPUT_ROOT / "outputs" / path_obj.name)
...
async def generate_openai_image_bytes(prompt: str, aspect_ratio: str = "16:9", output_path: str | None = None) -> tuple[bytes, str, str, str]:
    """调用 OpenAI 兼容 images API 生图并返回 (bytes, content_type, file_path, size)。"""
    if not OPENAI_IMAGE_API_KEY:
        raise ValueError("OPENAI_IMAGE_API_KEY 未配置")
    if not OPENAI_IMAGE_BASE_URL:
        raise ValueError("OPENAI_IMAGE_BASE_URL 未配置")
    if not OPENAI_IMAGE_MODEL:
        raise ValueError("OPENAI_IMAGE_MODEL 未配置")

    size = IMAGE_SIZE_MAP.get(aspect_ratio, "1536x864")
    output_path = resolve_output_path(output_path)

    async with httpx.AsyncClient(timeout=120.0) as image_client:
    ...
    if resp.status_code != 200:
        raise RuntimeError(f"OpenAI-compatible image API HTTP {resp.status_code}: {resp.text[:500]}")

    body = resp.json()
    data_list = body.get("data") or []
    if not data_list or not data_list[0].get("b64_json"):
        raise RuntimeError(f"OpenAI-compatible image API 返回无图片数据: {json.dumps(body, ensure_ascii=False)[:500]}")
...
async def prepare_wechat_image(arguments: dict, token: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        if source_type == "generate":
            ...
            image_data, content_type, file_path, generated_size = await generate_openai_image_bytes(
                prompt=prompt,
...
async def list_tools():
    return [
        ...
        Tool(
            name="mcp_openai_generate_image",
            description="使用 OpenAI 兼容 images API 生成图片。输入提示词和可选参数，返回生成图片的本地文件路径。",
            inputSchema={
                "type": "object",
                "required": ["prompt"],
                "properties": {
...
async def call_tool(name: str, arguments: dict):
    ...
    # ── OpenAI 兼容文生图（不需要微信 token）──────────────────
    if name == "mcp_openai_generate_image":
        try:
            image_data, content_type, output_path, size = await generate_openai_image_bytes(
